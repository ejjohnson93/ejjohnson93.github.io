...
Growing up, Ava Lovelace received tutoring in mathematics. Her mother encouraged this, not just because she strongly believed in women's education, but also to counter the dangerous poetic tendencies of her father. Her legacy is one that sees the beauty in . In this activity we bring programming together with poetry. The aim of this activity is to print and format a poem using python.
...
There is a python package called 'time'. Python packages contain additional commands to allow us to carry out certain tasks. 'Time' contains a function called `sleep` that will allow us to add a pause between each line.

An example usage of sleep is below:
...
If we hover over the names of the functions replit will give us some information about that function.
...
You might wonder how this could be used in later work. Many games are written using python as a programming language. These might have blocks of text or dialogue that we might want to space out and format. You could use the skills learned here to do so. Alternatively, when writing software, programmers often like to include quotes as outputs to keep their users entertained and show that the software is doing something.
